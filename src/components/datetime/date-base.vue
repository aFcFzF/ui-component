<template>
<div class="ui-date-content">
    <div class="ui-date-header" v-if="type != 'time'">
        <span class="ui-date-year-left-picker" @click.stop="updateView('default', -1)">
            <i class="ui-icon-left"></i>
            <i class="ui-icon-left"></i>
        </span>
        <span class="ui-date-montui-left-picker" @click.stop="updateView('month', -1)" v-show="view=='date'||view=='week'">
            <i class="ui-icon-left"></i>
        </span>
        <span class="ui-date-header-show" @click.stop="changeView('year')" v-if="view != 'year'">
            {{nowView.year()}}年
        </span>
        <span class="ui-date-header-show" v-if="view == 'year'">
            {{nowView.year()-6}}&nbsp;&nbsp;-&nbsp;&nbsp;{{nowView.year()+5}}年
        </span>
        <span class="ui-date-header-show" @click.stop="changeView('month')" v-show="view != 'year' && view != 'month' && view != 'quarter'">
            {{months[nowView.month() - 1]}}
        </span>
        <span class="ui-date-header-show" @click.stop="changeView('date')" v-show="view == 'hour' || view == 'minute'">
            {{nowView.date()}}日
        </span>
        <span class="ui-date-year-right-picker" @click.stop="updateView('default', 1)">
            <i class="ui-icon-right"></i><i class="ui-icon-right"></i>
        </span>
        <span class="ui-date-montui-right-picker" @click.stop="updateView('month', 1)" v-show="view=='date'||view=='week'">
            <i class="ui-icon-right"></i>
        </span>
    </div>
    <div class="ui-date-header" v-show="view=='minute'">
        <span class="ui-date-montui-left-picker" @click.stop="updateView('hour', -1)"><i class="ui-icon-left"></i></span>
        <span class="ui-date-header-show" @click.stop="changeView('hour')">{{nowView | hoursString}}</span>
        <span class="ui-date-montui-right-picker" @click.stop="updateView('hour', 1)"><i class="ui-icon-right"></i></span>
    </div>
    <div :class="dateBodyCls">
        <div class="ui-date-body-weeks" v-if="view=='date'"><span v-for="w of weeks" :key="w">{{w}}</span></div>
        <div class="ui-date-body-pickers">
            <span v-for="d of dates" :key="d.string" :string="d.string" :class="{'ui-date-not-now-day': !d.isNowDays, 'ui-date-today':d.isToday, 'ui-date-selected': isSelected(d), 'ui-date-range-selected': isRangeSelected(d), 'ui-date-disabled': d.disabled}" @click.stop="chooseDate(d)">{{d.show}}</span></div>
    </div>
</div>
</template>
...
const genData = param => {
    ...
    if (utils.isObject(vm.option)) {
        vm.option.start && (disabled = date.distance(vm.option.start, type) < 0);
        vm.option.end && !disabled && (disabled = date.distance(vm.option.end, type) > 0);
        vm.option.disabled && !disabled && (disabled = vm.option.disabled.call(null, date));
    }
    let dis = date.distance(vm.today, type);
    let isToday = dis === 0;
    view === 'quarter' && (isToday = dis >= -2 && dis <= 0);
    return {
        date,
        show,
...
export default {
    ...
    filters: {
        hoursString(d) {
            const l = utils.padLeft(d.hours(), 2);
            return `${l}:00`;
        }
    },
    watch: {
                    ...
                    event => {
                        if (this.view === 'date' && !!this.value.start && !this.value.end) {
                            ...
                            if (target.tagName === 'SPAN') {
                                ...
                                string && this.$emit('updateRangeEnd', string);
                            }
                        }
                    },
                    true
    ...
    methods: {
        ...
        chooseDate(d) {
            if (this.view === endView[this.type]) {
                ...
                if (!(this.view === 'month' && this.type !== 'month')
                    ...
                    if (
                        this.options.start
                        && manba(date).time() < manba(this.options.start).time()
                    ) {
                    ...
                    if (
                        this.options.end
                        && manba(date).time() > manba(this.options.end).time()
                    ) {
    ...
    computed: {
        ...
        dates() {
            ...
            if (this.view === 'date') {
                ...
                let dates = [];
                let list = [];
                utils.isFunction(this.options.hours) ? list = this.options.hours.call(null)
                ...
                while (date.month() === month || date.month() === nextMonth) {
                    const show = date.year() + '年 第'
                        + index + '周' + date.format('MM-DD')
                        + ' 至 ' + manba(date).add(6).format('MM-DD');
                    dates.push(
                        genData({
                    ...
                    date = date.add(7);
                    oldYear !== date.year() ? index = 1 : index += 1;
                }
                return dates;
            }
