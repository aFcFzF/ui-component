<template>
<div :class="dateCls">
    <div v-if="noBorder" class="ui-datetime-show text-hover">{{showValue||placeholder}}</div>
    <div v-else class="ui-input ui-datetime-show">
        <input type="text" :value="showValue" readonly :placeholder="showPlaceholder" />
        <i class="ui-icon-calendar"></i>
    </div>
    <div :class="datePickerCls" class="ui-date-picker">
        <div class="ui-date-container ui-date-full-range-container">
            <div v-if="shortcuts.length>0" class="ui-date-shortcut">
            ...
            <div v-if="view == 'date'" class="ui-date-self-defined">
            ...
            <date-base v-else ref="datebase" :value="nowDate.start" :option="option" :type="view" :startWeek="startWeek" :now-view="nowView.start" format="k" @updateView="updateView" @input="setvalue" @changeView="updateDropdown"></date-base>
        </div>

        <div class="ui-date-footer">
            <button class="ui-btn ui-btn-text ui-btn-s" @click="clear">{{'h.common.clear' | hlang}}</button>
            <button class="ui-btn ui-btn-primary ui-btn-s" @click="confirm">{{'h.common.confirm' | hlang}}</button>
        </div>
    </div>
</div>
</template>
...
export default {
    ...
    mounted() {
        let that = this;
        this.$nextTick(() => {
            let el = this.el = this.$el.querySelector(`.${prefix}>.ui-datetime-show`);
            let content = this.$el.querySelector('.ui-date-picker');
            this.dropdown = new Dropdown(el, {
                ...
                events: {
                    show() {
                        that.isShow = true;
                        that.$nextTick(() => that.initNowView())
                    }
                }
            });
        });
    ...
    methods: {
        ...
        initNowView() {
            let start = manba();
            !!this.nowDate.start && (start = manba(this.nowDate.start));
            let end = manba();
            !!this.nowDate.end && (end = manba(this.nowDate.end));
            let endRange = 1;
            this.nowView = {
                start,
                end
        ...
        setvalue(string = '', isEnd = false) {
            if (this.view === 'date') {
                const value = utils.copy(this.nowDate);
                value.end && (value.end = manba(value.end).add(1).format(this.nowFormat));
    ...
    computed: {
        ...
        showValue() {
            ...
            return `${this.value.start || '起始日期'} - ${this.value.end?manba(this.value.end).add(-1).format(this.nowFormat): '结束日期'}`;
        },
        shortcuts() {
            let shortcuts = [];
