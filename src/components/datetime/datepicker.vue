...
<div :class="dateCls">
    <template v-if="!inline">
        <div v-if="noBorder" class="ui-datetime-show text-hover">{{showDate||showPlaceholder}}</div>
    ...
    <div :class="datePickerCls" class="ui-date-picker">
        <div class="ui-date-container" v-if="isShow">
        ...
        <div class="ui-date-footer" v-if="hasConfirm & !inline">
            <button class="ui-btn ui-btn-text" @click="clear">{{'h.common.clear' | hlang}}</button>
            <button class="ui-btn ui-btn-primary ui-btn-s" @click="hide">{{'h.common.confirm' | hlang}}</button>
        </div>
    </div>
</div>
</template>
...
export default {
    ...
    methods: {
        ...
        updateView(value) {
            ...
            this.$nextTick(() => this.updateDropdown());
        },
        updateDropdown() {
        ...
        changeEvent(event) {
            ...
            if (this.nowDate && utils.isObject(this.option) && this.type !== 'time') {
                ...
                this.option.start && nowDate.distance(this.option.start, type) < 0 && (disabled = this.option.start);
                this.option.end && !disabled && nowDate.distance(this.option.end, type) > 0
                && (disabled = this.option.end);
                this.option.disabled && this.option.disabled.call(null, disabled || nowDate) && (disabled = '');
                disabled !== false && this.parse(disabled);
            }
            this.setvalue(this.nowDate);
        },
                ...
                try {
                    ...
                    if (initShow) {
                        if (this.type === 'week') {
                            debugger;
                            this.showDate = {
                                year: '第' + this.nowView.year() + '年',
                                week: '第' + this.nowView.getWeekOfYear(manba.MONDAY) + '周'
                            };
                        }
                            ...
                            this.showDate = {
                                year: '第' + this.nowView.year() + '年',
                                quarter: '第' + Math.ceil(this.nowView.month() / 3) + '季度'
                            };
                        }
                        else {
        ...
        shortcuts() {
            ...
            if (utils.isArray(shortcutsConfig)) {
                for (let s of shortcutsConfig) {
                    utils.isString(s) && shortcuts.push(options.shortcuts[s]);
                    utils.isObject(s) && shortcuts.push(s);
                }
            }
            return shortcuts;
        },
