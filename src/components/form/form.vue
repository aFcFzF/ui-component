...
const prefixCls = 'ui-form';

const findDomUtil = function (target, utilDom) {
    let now = target;
    while (now !== utilDom) {
        if (utils.hasClass(now, 'ui-form-item') && now.getAttribute('prop')) {
            return now;
        }
...
export default {
    ...
    beforeMount() {
        ...
        this.model && this.rules && (this.validator = new Validator(this.rules));
    },
    mounted() {
    ...
    watch: {
        rules: {
            handler() {
                this.validator && this.rules && this.validator.updateRule(this.rules);
                (this.model && this.rules) && (this.validator = new Validator(this.rules));
            },
            deep: true
        }
    ...
    methods: {
        ...
        renderMessage(returnResult) {
            ...
            if (!isSuccess) {
                this.$nextTick(() => {
                    let firstError = this.$el.querySelector('.ui-form-item-valid-error');
                    if (firstError) {
            ...
            return {
                result: isSuccess,
                messages: utils.toArray(this.messages).filter(item => !item.valid)
            };
        },
        validAsync() {
