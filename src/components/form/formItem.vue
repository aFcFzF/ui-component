...
<div :class="formItemCls" :prop="prop" :validable="validable">
    <label :style="labelStyleCls" class="ui-form-item-label" v-if="showLabel">{{label}}</label>
    <div class="ui-form-item-content" :style="contentStyleCls">
        <div class="ui-form-item-wrap">
            <slot></slot>
        </div>
        <div class="ui-form-item-error" v-if="!errorMessage.valid">
            <span v-if="errorMessage.type=='base'" class="ui-form-item-error-label">{{label}}</span>
...
export default {
    ...
    methods: {
        getParent() {
            ...
            let filterTag = new Set(['Form', 'uiForm', 'ui-form']);
            while (parent != null && !filterTag.has(parent.$options._componentTag)) {
                parent = parent.$parent;
            }
            if (!parent) {
                console.error('ui-form-item 必须放在ui-form里面才可以生效');
            }
            return parent;
        },
        getDirectParent() {
            let parent = this.$parent;
            let filterTag = new Set(['Form', 'ui-form', 'ui-form-item', 'FormItem']);
            while (parent != null && !filterTag.has(parent.$options._componentTag)) {
                parent = parent.$parent;
            }
            if (!parent) {
