<template>
    <div>
        <div :class="noticeCls">
            <div class="ui-notify-mask"
                v-if="hasMask"
                @click="setvalue(true)"></div>
            <div :class="{'ui-notify-body': !!hasMask}" @click.self="setvalue(true)">
                <div :class="containerCls">
                <span
                    class="ui-notify-close ui-icon-close"
...
export default {
    props: {
        ...
        hasDivider: {
            type: Boolean,
            default() {
                config.getOption('modal', 'hasDivider');
            }
        },
        closeOnMask: {
            type: Boolean,
    ...
    watch: {
        value() {
            if (this.value) {
                ...
                setTimeout(() => this.isOpened = this.value, 100);
            }
            else {
    ...
    mounted() {
        this.$nextTick(() => {
            const el = this.el = this.$el.firstChild;
