  ...
  <th :class="cls" @click="triggerSort()">
    <div v-if="tooltip" v-tooltip :placement="placement" :content="content || title">
      ...
      <span class="h-table-sort-handler" v-if="sort">
        <span class="h-table-sort-asc" v-if="sortStatus.type == 'asc' && sortStatus.prop == prop" :class="{'sort-selected': sortStatus.type == 'asc' && sortStatus.prop == prop}"><i class="ui-icon-top"></i></span>
        <span class="h-table-sort-desc" v-else :class="{'sort-selected': sortStatus.type == 'desc' && sortStatus.prop == prop}"><i class="ui-icon-down"></i></span>
      </span>
    </div>
    <div v-else>
      <span>{{title}}</span>
