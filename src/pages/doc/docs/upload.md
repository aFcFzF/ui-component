    ...
    data() {
      return {
        options: {
          ...
          filters: {
            mime_type: [
              {title: 'img_files', extentions: 'jpg,gif,png'}
            ]
          }
        },
        uploadList: [],
...
```html
<ui-uploader :upload-list="uploadList" :options="options" :type="type" v-model="file">
  <template slot="dragdrop">
    <ui-button class="ui-btn-primary" >点击上传</ui-button>
    <div class="ui-uploader__tip">只能上传jpg/png文件，且不超过500kb</div>
  </template>
</ui-uploader>
<script>
  export default {
