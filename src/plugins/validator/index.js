...
import utils from '../../utils/utils_ext.js';
...
export default class Validator {
    constructor(rules) {
        !utils.isObject(rules) && console.error('validator: please pass the correct validation parameters');
        this.rules = {};
        this.combineRules = {};
        ...
        for (let v of keys) {
            ...
            if (utils.isArray(validList)) {
                for (let p of validList) {
                    ...
                    !utils.isObject(rule) && (rule = genRules.rules[p] = {});
                    v === 'required' ? rule.required = true : rule.type = v;
    ...
    initCombineRules(rules) {
        ...
        for (let rule of rules) {
            let parentRef = '';
            rule.parentRef && (parentRef = `${rule.parentRef}.`);
    ...
    valid(data, next, allNext) {
        ...
        let result = this.validData(data, function (r) {
            for (let key in r) {
                loadings.indexOf(key) > -1 && loadings.splice(loadings.indexOf(key), 1);
            }
        ...
        for (let prop in result) {
            result[prop].loading && loadings.push(prop);
        }
        if (allNext && loadings.length === 0) {
            allNext.call(this, result);
        }
